    ...
    success: function(jsonData) {
      ...
      $('#walBox').onepage_scroll({
        ...
        afterMove: function(index) {
          ...
          // $(".section").eq(index-1).attr('less','background-image: url('+ $(".section").eq(index-1).attr('data-img') +')');
        }
      })
...
function ajaxCiba(data) {
  $.ajax({
    type: 'GET',
    url: seting.apiUrl + '?cid=English',
    // data: "cid=bing&start=" + start + "&count=" + count,
    dataType: 'json',
    success: function(jsonData) {
      var newHtml = '<less>#walBox .note{position: fixed;}body{background-image: url(' + jsonData.picture2 + ');overflow: hidden;}</less>' +
        ...
        ' <span title="' + jsonData.love + '人喜欢" class="ciba-love" onclick="$(\'.love-count\').html(parseInt($(\'.love-count\').html()) + 1)"><span less="color: red;">❤</span>+<span class="love-count">' + jsonData.love + '</span></span></p>' +
